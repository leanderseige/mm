void mmef_newresource_video() {
  ...
  dialog=gtk_dialog_new_with_buttons( "Video laden", GTK_WINDOW(fenster), GTK_DIALOG_MODAL,GTK_STOCK_OK, GTK_RESPONSE_ACCEPT,GTK_STOCK_CANCEL,GTK_RESPONSE_REJECT,NULL);
  ...
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(uni_spin01),0.4);
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(uni_spin02),0.3);
  ...
  switch (result) {
    ...
    case GTK_RESPONSE_NONE:
      ...
      h=gtk_spin_button_get_value_as_float(GTK_SPIN_BUTTON(uni_spin01));
      if(path) {
        if(MM_RROW_FocusResource=mmf_add_video((char *)path,1,w,h,(char *)text)) {
